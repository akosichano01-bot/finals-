...
const Payments = () => {
  // Laging i-initialize bilang empty array []
  ...
  useEffect(() => {
    const fetchPayments = async () => {
      try {
        ...
        // Kunin ang token mula sa localStorage
        const token = localStorage.getItem('token'); 
        
        const res = await axios.get('/api/payments', {
          headers: { Authorization: `Bearer ${token}` } // Importante para sa 401 error
        });
        ...
        setPayments([]); // Fallback para hindi mag-crash ang .map()
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="p-6 bg-[#1a1c23] min-h-screen text-white">
      ...
      <div className="overflow-x-auto bg-[#24262d] rounded-xl shadow-lg">
        <table className="w-full text-left">
          ...
          <tbody>
            {/* Siguraduhin na payments ay array bago i-map */}
            {payments.length > 0 ? (
              payments.map((p) => (
                <tr key={p.id} className="border-b border-gray-800 hover:bg-gray-700/30">
                  <td className="p-4">{p.tenant_name || 'N/A'}</td>
                  <td className="p-4 text-xs text-gray-400">{p.bill_type}</td>
                  <td className="p-4 text-green-400">₱{Number(p.amount).toLocaleString()}</td>
                  <td className="p-4">
